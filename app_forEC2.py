...
def submit():
 ...
 if contact_list:
     ...
     if len(station_table) >= 1:
      ...
      df_new['Station2'] = station
        
      # RENAMING COLUMNS IN DATAFRAME
      
      df_new.rename(columns={ df_new.columns[7]: "Revenue" }, inplace = True)
      df_new.rename(columns={ df_new.columns[0]: "Market_station" }, inplace = True)

       
      df_new['Market'] = df_new['Station'].astype(str).str[-3:]
      df_new['Level'] = df_new['Market_station'].astype(str).str[3:] 
     ...
     if len(station_table) >= 2:
        ...
        df_new_1.rename(columns={ df_new.columns[7]: "Revenue" }, inplace = True)
     ...
     if len(station_table) == 4:
         ...
         # AFTER ALL THE IF STATEMENTS, BELOW ADD IF REQUEST.METHOD == 'POST': print(df_new or final_df) and return template with message 'Submitted to Milton Data'
