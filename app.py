...
class DropdownForm_SEN(FlaskForm):
    ...
    type2 = SelectField(u'Month Code', choices = [(0,'Select'),('2022M01', '2022M01'), ('2022M02', '2022M02'), ('2022M03','2022M03'), ('2022M04', '2022M04'), ('2022M05', '2022M05'), ('2022M06', '2022M06'), ('2022M07', '2022M07'), ('2022M08', '2022M08'), ('2022M09', '2022M09'), ('2022M10', '2022M10'), ('2022M11', '2022M11'), ('2022M12', '2022M12')],  default='2022M04')
...
def edit():
 ...
 if current_user.network == 'SEN':
        ...
        networks.append("NineStreaming-Nat")
        networks.append("NinePodcast-Nat")
         ...
         if name == ( month + "-" + network + "-" + contact + "-" + "0" +  ".db"):
          ...
          if form.fillzeroes.data:
           ...
           if station in station_edit_Mel_Syd:
            ...
            form.PerTotal.data = Decimal(0)
           elif station in station_edit_Bri:
 ...
 if len(list_st) >=1 :
     ...
     if df is not None:
      if len(incomplete) != 0:
       ...
       return render_template("review.html", form=form, form_1 = form_1, user_image=pic, network=current_user.network, contact=contact, message = message,  header = header, tables=[df.to_html(classes='data', header="true")], tables_incomplete = [df_incomplete.to_html(classes='incomplete')])
      else:
...
def reporting():
 ...
 if request.method == 'POST':
    ...
    mkt_current = dataset[['RevenueGroup', 'MktCurrent']]
    stn_current = dataset[['RevenueGroup', 'StnCurrent']]
    mkt_growth = dataset[['RevenueGroup', 'MktGrowth%']]
    stn_growth = dataset[['RevenueGroup', 'StnGrowth%']]
    
    

  
    labels = [row[0] for row in mkt_current.values]
    values = [row[1] for row in mkt_current.values]
   
    labels_growth = [row[0] for row in mkt_growth.values]
    values_growth = [row[1] for row in mkt_growth.values]

    labels_stn_growth = [row[0] for row in stn_growth.values]
    values_stn_growth = [row[1] for row in stn_growth.values]

    labels_stn_current = [row[0] for row in stn_current.values]
    values_stn_current = [row[1] for row in stn_current.values]

    market_current_vs_station_current = "Market Current vs Station Current"

    market_growth_vs_station_growth = "Market Growth vs Station Growth"
     
    return render_template("reporting.html", form=form, user_image=pic, network=current_user.network, period = form.period_select.data, market_current_vs_station_current = market_current_vs_station_current, market_growth_vs_station_growth = market_growth_vs_station_growth,  month = form.calendar_month.data, station = form.type4.data, link = link, tables=[dataset.to_html(classes='data', header="true")], labels = labels, values = values, labels_growth = labels_growth, values_growth = values_growth, labels_stn_growth = labels_stn_growth, values_stn_growth = values_stn_growth, labels_stn_current = labels_stn_current, values_stn_current = values_stn_current) 
