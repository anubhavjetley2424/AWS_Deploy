...
def reporting():
 ...
 if request.method == 'POST':
    ...
    fig = px.bar(market_growth, x= market_growth['RevenueGroup'], y= market_growth['MktGrowth%'],
     labels={
                     "value": "MktGrowth%",
                     "variable": "Market",
                    
                 }, template= 'plotly_dark', title='Mkt Growth %', width=1200, height=600)
    ...
    return render_template("reporting.html", form=form, user_image=pic, network=current_user.network, period = form.period_select.data, month = form.calendar_month.data, station = form.type4.data, link = link, tables=[dataset.to_html(classes='data', header="true")], graphJSON = graphJSON) 
