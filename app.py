...
def review():
 ...
 if current_user.network == 'SEN':
  if len(list_st_SEN) >=1 :
     station = list_st_SEN[-1]
     if station == "":
         station = list[-2]
 if current_user.network == 'TAB':
  if len(list_st_TAB) >=1 :
     station = list_st_TAB[-1]
     if station == "":
         station = list[-2]
 if current_user.network == 'SCA':
  if len(list_st_SCA) >=1 :
     station = list_st_SCA[-1]
     if station == "":
         station = list[-2]
 if current_user.network == 'NineRadio':
  if len(list_st_Nine) >=1 :
     station = list_st_Nine[-1]
     if station == "":
         station = list[-2]
 if current_user.network == 'Nova':
  if len(list_st_Nova) >=1 :
     station = list_st_Nova[-1]
     if station == "":
         station = list[-2]
 if current_user.network == 'ARN':
  if len(list_st_ARN) >=1 :
     ...
     if station == "":
         station = list[-2]
 if current_user.network == 'ACE':
  if len(list_st_ACE) >=1 :
     station = list_st_ACE[-1]
     if station == "":
         station = list[-2]
 if current_user.network == '2SM':
  if len(list_st_2SM) >=1 :
     station = list_st_2SM[-1]
     if station == "":
         station = list[-2]
 if current_user.network == 'RadioSport':
  if len(list_st_RadioSport) >=1 :
     station = list_st_RadioSport[-1]
     if station == "":
         station = list[-2]
 if current_user.network == '6IX':
  if len(list_st_6IX) >=1 :
     station = list_st_6IX[-1]
     if station == "":
         station = list[-2]
     ...
     for x in range(1,18):
      ...
      if current_user.network == 'SEN':
        if len(list_st_SEN) >= index:
          month = var_list[-index]
          station = list_st_SEN[-index]
      if current_user.network == 'TAB':
       if len(list_st_TAB) >= index:
        month = var_list[-index]
        station = list_st_TAB[-index]
      if current_user.network == 'SCA':
       if len(list_st_SCA) >= index:
        month = var_list[-index]
        station = list_st_SCA[-index]
      if current_user.network == 'NineRadio':
       if len(list_st_Nine) >= index:
        month = var_list[-index]
        station = list_st_Nine[-index]
      if current_user.network == 'Nova':
       if len(list_st_Nova) >= index:
        month = var_list[-index]
        station = list_st_Nova[-index]
      if current_user.network == 'ARN':
       if len(list_st_ARN) >= index:
        month = var_list[-index]
        station = list_st_ARN[-index]
      if current_user.network == 'ACE':
       if len(list_st_ACE) >= index:
        month = var_list[-index]
        station = list_st_ACE[-index]
      if current_user.network == '2SM':
       if len(list_st_2SM) >= index:
        month = var_list[-index]
        station = list_st_2SM[-index]
      if current_user.network == 'RadioSport':
       if len(list_st_RadioSport) >= index:
        month = var_list[-index]
        station = list_st_RadioSport[-index]
      if current_user.network == '6IX':
       if len(list_st_6IX) >= index:
        month = var_list[-index]
        station = list_st_6IX[-index]
...
if __name__ == 'main':
  ...
  app.run(debug=True, port=5000, host='0.0.0.0', threaded=True, processes=10)