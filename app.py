      ...
      if len(list_st) >= index:
       ...
       if station in review_station_list:
        ...
        if cursor.fetchone()[0]==1 :
         ...
         if station in station_edit_Metro:
          row_df = pd.DataFrame(data = l, columns = ['', 'MetroTotal'])
...
def submit():
 ...
 if list_st:
     ...
     def submit_table(station_function, month_function):
        ...
        df_new_1['Level'] = df_new_1['Level'].replace({'roTotal' : 'Total'}, regex=True)
        df_new_1['Market'] = df_new_1['Market'].replace({'Met' : 'Metro'}, regex=True)
