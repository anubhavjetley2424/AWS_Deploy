...
def submit():
 ...
 if contact_list:
     ...
     def table_to_csv(table_milton, df):
      
        x = dt.datetime.now()
        contact_new = contact.replace(" ","-")
        base_path = os.getcwd()

        name_submit = month + "-" + network + "-" + contact_new + "-" + str(x.year) + "-" + str(x.month) + "-" + str(x.day) + "-" + str(x.hour) +  "-" + str(x.minute)
        ...
        s3_resource.Object(bucket, name_submit + '.csv').put(Body=csv_buf.getvalue())

        sql_path = os.path.join(base_path + "\\" + name_submit + ".sqlite")
        conn = sqlite3.connect(sql_file)
      
        table_milton.to_sql(sql_file, conn, if_exists='append', index = False)

      
        s3 = boto3.resource('s3')
        s3.meta.client.upload_file(sql_path, 'miltondata-radio1', name_submit + ".sqlite")
     ...
     if request.method == 'POST':
       ...
       table_to_csv(table_milton, table)
       print(table_milton)
